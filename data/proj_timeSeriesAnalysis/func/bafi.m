...
%   lofi.m or hifi.m for aditional information
% 
%                         >>> INPUT VARIABLES >>>
% NAME              TYPE/DEFAULT         DESCRIPTION
% d                 array (up to 3d)     data to be filtered (along columns!)
...
try
  ...
  % multiplied and added to efreq below to make things converge. will be
  ...
  while ~finito
    ...
    % a) hipass: 
    % The -1.5 dB freq should be located between cfreq and the -3 dB freq of the 
    % current run, so investigate this range
    ...
    % b) lopass: The -1.5 dB freq should be located between
    ...
    % if both efreq values are close to each other a static stepFac may lead to
    % too fast a convergence - avoid that by making sure that fdev*stepFac never
    % exceeds .5 of the frequency span of the current efreq
    ...
    % also, if sign of fdev is any different from [+ -], we overshot the goal and 
